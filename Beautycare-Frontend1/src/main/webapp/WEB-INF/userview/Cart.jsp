...
<div > 
					<h3 style="font-family:monotype corsiva;color:black;font-size: 1.7em;"><b>Items in your cart</b></h3>
					</div>
<div  ng-app="addToCartApp"  ng-controller = "addToCartCtrl"  ng-init="retrieveCart('${cartId}')">
	 
	 <div >
        <table class="table table-bordered" style="font-family:monotype corsiva;color:black;font-size: 1.5em;" >
		  <thead>
			  <tr>
			  <th>Product</th>
				<th >Unit Price</th>
			...
			<tbody>
				<tr ng-repeat = "items in cart.cartItems">
						...
						<td><button class="btn btn-danger btnAction" ng-click="removeItemFromCart('${items.totalPrice}')">
 ...
 cartApp.controller("addToCartCtrl", function ($scope, $http){
      ...
      */
      $scope.removeItemFromCart = function (id) {
         $http['delete']('http://localhost:8080/beautycare/categoryDropdown/productDisplay/usercart/cart/removeItem/'+id).success(function (data) {
     ...
     $scope.addItemToCart = function (id) {
         $http.put('http://localhost:8080/beautycare/usercart/cart/addItem/'+id).success(function (data) {
     ...
     $scope.refreshCartItems = function () {
         $http.get('http://localhost:8080/beautycare/categoryDropdown/productDisplay/usercart/cart/refreshCart/'+$scope.cartId).success(function (data) {
            $scope.cart=data;
         });
     };
     ...
     $scope.clearCartItems = function () {
     	$http['delete']('http://localhost:8080/beautycare/categoryDropdown/productDisplay/usercart/cart/clearCartItems/'+$scope.cartId).success(function (data)
     			{$scope.refreshCartItems()
     		});
     };
 });
